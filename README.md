...
目前版本：`0.9.20`。首頁先顯示「今日必看 3 件事」：台中緊急事件與重大持股訊息優先，再來才是 AI 等一般情報。關注頻道預設收合，點中文分類才展開，避免手機一直滑。未翻譯的英文社群貼文與 RSS HTML 片段不會當成首頁內容；沒有可靠資料時會誠實顯示，而不以無關內容湊數。圖片只保留可用的 YouTube 官方縮圖；圖片失敗時整張卡與空區塊都會移除。頁首顯示本次與下一次預計更新時間；「有興趣／少一點」會記在同一支手機，讓下次開啟時保留個別偏好效果。每次 Sprint 完成會同步更新 `VERSION` 與 `CHANGELOG.md`；Git commit 可作為可回復版本點。
