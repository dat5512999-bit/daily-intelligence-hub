...
目前版本：`0.9.18`。首頁採混合式 Dashboard：Hero 先告訴你今天最值得看什麼，突發事件使用 Alert，熱度訊號用排行榜，持股與版本更新用短清單，活動用時間軸；不再把所有資訊塞進同一種新聞卡。YouTube 官方縮圖與 GitHub Trending 的 Owner Avatar 才會顯示圖片，其餘內容不任意配圖。頁首也會顯示「來源正常／部分來源受限／資訊來源不足／資料不足」，不會在只剩少量媒體來源時假裝即時連線正常。每次 Sprint 完成會同步更新 `VERSION` 與 `CHANGELOG.md`；Git commit 可作為可回復版本點。
